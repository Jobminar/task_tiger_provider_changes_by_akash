...
function intlFormatDistance(laterDate, earlierDate, options) {
  ...
  const rtf = new Intl.RelativeTimeFormat(options?.locale, __spreadValues({
    numeric: "auto"
  }, options));
...
function isSameISOWeek(laterDate, earlierDate, options) {
  return isSameWeek(laterDate, earlierDate, __spreadProps(__spreadValues({}, options), { weekStartsOn: 1 }));
}
...
function lastDayOfISOWeek(date, options) {
  return lastDayOfWeek(date, __spreadProps(__spreadValues({}, options), { weekStartsOn: 1 }));
}
